...
const excludedExtensions = [
	...
	// CoAuthor: Exclude coding-related extensions for writing-focused app
	...
	// Keep: markdown-*, latex, restructuredtext (writing formats)
	// Keep: git-*, github-* (version control)
	// Keep: themes, media-preview, simple-browser (UI/preview)
	// Keep: coauthor-* (custom extensions)
	// Keep: json*, configuration-editing, diff, merge-conflict (utilities)
];
...
function doPackageLocalExtensionsStream(forWeb: boolean, disableMangle: boolean, native: boolean): Stream {
	...
	if (forWeb) {
		...
		// also include shared production node modules, but only from extensions that need them
		const needsDeps = localExtensionsDescriptions
			.filter(ext => ext.name !== 'coauthor-agent')
			.filter(ext => {
				// Treat as bundled if a dist build exists or if a webpack config is present
				const hasDist = fs.existsSync(path.join(ext.path, 'dist', 'extension.js'));
				const hasWebpack = fs.existsSync(path.join(ext.path, 'extension.webpack.config.js'))
					|| fs.existsSync(path.join(ext.path, 'extension-browser.webpack.config.js'));
				return !(hasDist || hasWebpack);
			});

		let dependenciesSrc: string[] = [];
		for (const ext of needsDeps) {
			for (const depRoot of getProductionDependencies(ext.path)) {
				const rel = path.relative(root, depRoot);
				dependenciesSrc.push(`${rel}/**`, `!${rel}/**/{test,tests}/**`);
			}
		}
